...
class crawl(scrapy.Spider):
    ...
    @staticmethod
    def prepare_urls():
        # data = open("data/step_7_category_oyuncak_product_url.json", encoding="utf-8").read()
        data = open("data/step_7_category_pratikev_product_url.json", encoding="utf-8").read()
    ...
    def start_requests(self):
        ...
        for url in urls:
            req = scrapy.Request(url, callback=self.continues, dont_filter=True, headers=self.headers)
    ...
    def close(self, spider, reason):
        # with open(f"data/step_8_oyuncak_basket_{self.name}.json", "w", encoding="utf-8") as f:
        #     json.dump(self.output, f, ensure_ascii=False)

        with open(f"data/step_8_pratikev_basket_{self.name}.json", "w", encoding="utf-8") as f:
            json.dump(self.output, f, ensure_ascii=False)
